...
Insert into REG_APP.EVENTS (ID,EVENT_NAME,EVENT_DESCRIPTION,EVENT_START_DATE,EVENT_PRICE,EVENT_END_DATE) values (1,'World Wide Trade','Contoso WWT is the best trade conference in the world. WWT allows import and exporters to connect with your community, and explore the latest technology.',TO_DATE('21-SEP-20','%e-%b-%y'),1200,TO_DATE('25-SEP-20','%e-%b-%y'));
-- INSERTING into REG_APP.REGISTRATIONS
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (1,TO_DATE('02-MAR-20','%e-%b-%y'),1,1);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (2,TO_DATE('03-FEB-20','%e-%b-%y'),1,2);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (5,TO_DATE('04-MAR-20','%e-%b-%y'),2,3);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (6,TO_DATE('18-MAR-20','%e-%b-%y'),2,6);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (24,TO_DATE('16-MAR-20','%e-%b-%y'),2,52);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (26,TO_DATE('16-MAR-20','%e-%b-%y'),2,41);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (27,TO_DATE('16-MAR-20','%e-%b-%y'),1,25);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (43,TO_DATE('24-MAR-20','%e-%b-%y'),2,114);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (63,TO_DATE('24-MAR-20','%e-%b-%y'),2,148);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (83,TO_DATE('25-MAR-20','%e-%b-%y'),2,56);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (85,TO_DATE('25-MAR-20','%e-%b-%y'),1,56);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (86,TO_DATE('25-MAR-20','%e-%b-%y'),2,136);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (87,TO_DATE('26-MAR-20','%e-%b-%y'),1,5);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (89,TO_DATE('26-MAR-20','%e-%b-%y'),2,5);
Insert into REG_APP.REGISTRATIONS (ID,REGISTRATION_DATE,SESSION_ID,ATTENDEE_ID) values (92,TO_DATE('26-MAR-20','%e-%b-%y'),1,95);
-- INSERTING into REG_APP.SESSIONS
Insert into REG_APP.SESSIONS (ID,NAME,DESCRIPTION,SESSION_DATE,SPEAKER_ID,EVENT_ID,DURATION) values (1,'What is the Future of Trade? Multilateral or Bilateral?','This panel will cover the “big picture” of global trade concerning the benefits and limits of multilateral trade negotiations, impact of plurilateral and bilateral agreements on the multilateral system, where we are now and prospects for global trade post COVID-19.',TO_DATE('21-SEP-20','%e-%b-%y'),1,1,60);
Insert into REG_APP.SESSIONS (ID,NAME,DESCRIPTION,SESSION_DATE,SPEAKER_ID,EVENT_ID,DURATION) values (2,'New to Trade Compliance?','You will also learn how transition can be made from Trade Compliance to Global Trade Compliance and the specific challenges involved',TO_DATE('21-SEP-20','%e-%b-%y'),2,1,60);
Insert into REG_APP.SESSIONS (ID,NAME,DESCRIPTION,SESSION_DATE,SPEAKER_ID,EVENT_ID,DURATION) values (3,'Supply Chain Logistics, Trade and Tech Talk','Are you at the forefront of technology when it comes to managing your customs and logistics data or are you worried about being left behind?',TO_DATE('29-MAR-19','%e-%b-%y'),3,1,60);
Insert into REG_APP.SESSIONS (ID,NAME,DESCRIPTION,SESSION_DATE,SPEAKER_ID,EVENT_ID,DURATION) values (4,'U.S. Trade Policies and Countries of Production in a Global Supply Chain','Trade policies and actions such as Section 232 and Section 301, even before COVID-19, have affected US importers, global manufacturers, and US resellers decisions about global sourcing and countries of production.',TO_DATE('29-MAR-19','%e-%b-%y'),4,1,60);
...
-- Full Text Search
CREATE TEXT SEARCH CONFIGURATION reg_app."FTS_Main" (
	PARSER = default
);

CREATE TEXT SEARCH DICTIONARY reg_app."FTS_Main_Dict" (
    TEMPLATE = simple
);

CREATE TEXT SEARCH PARSER reg_app."FTS_Main_Parser" (
    START = prsd_start,
    GETTOKEN = prsd_nexttoken,
    END = prsd_end,
    LEXTYPES = dsimple_init,
    HEADLINE = prsd_headline);

CREATE TEXT SEARCH TEMPLATE reg_app."FTS_Main_Template" (
    INIT = dsimple_init,    LEXIZE = dispell_lexize
);
...
$$
BEGIN
    INSERT INTO Reg_App.Attendees_Audit ( "Id", "LastName", "FirstName","EmailAddress", "AuditTime")
...
-- create function (plperl)
CREATE FUNCTION perl_max (integer, integer) RETURNS integer AS $$
    if ($_[0] > $_[1]) { return $_[0]; }
    return $_[1];
$$ LANGUAGE plperl;

-- create foreigh data wrapper
CREATE EXTENSION postgres_fdw;
CREATE EXTENSION file_fdw;

CREATE SERVER app_database_server
  FOREIGN DATA WRAPPER postgres_fdw
  OPTIONS (host 'localhost', dbname 'reg_app');
  
  create USER MAPPING FOR CURRENT_USER
  SERVER app_database_server
  OPTIONS (user 'postgres', password 'Seattle123');
...
-- create perl language
CREATE OR REPLACE TRUSTED PROCEDURAL LANGUAGE plperl
    HANDLER plperl_call_handler
    INLINE plperl_inline_handler
    VALIDATOR plperl_validator;

ALTER LANGUAGE plperl OWNER TO postgres;

-- Create publication
CREATE PUBLICATION app_pub WITH (publish = 'insert, update, delete');

--create a cast
CREATE CAST (integer AS integer) WITH FUNCTION pymax(a integer, b integer);
...
--  Constraints for Table REGISTRATIONS
  ...
  ALTER TABLE REG_APP.REGISTRATIONS ALTER COLUMN ID SET DEFAULT nextval('REG_APP.reg_app_seq');
  ALTER SEQUENCE REG_APP.reg_app_seq RESTART WITH 103;
